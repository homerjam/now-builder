...
const { createLambda } = require('@homerjam/build-utils/lambda')
const download = require('@homerjam/build-utils/fs/download')
const FileFsRef = require('@homerjam/build-utils/file-fs-ref')
const FileBlob = require('@homerjam/build-utils/file-blob')
...
async function build({ files, entrypoint, workPath, config = {} }) {
  ...
  if (isYarn) {
    ...
    await exec('npm', [ 'install' ], { env: { NODE_ENV: 'production' } })
  }
